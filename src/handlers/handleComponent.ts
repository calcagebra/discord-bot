...
import deleteMessageButton from "../buttons/delete-message.js";
...
export async function handleComponent({ api, env, interaction }: HandleComponentOptions): Promise<Response> {
  try {
    if (interaction.data.custom_id === deleteMessageButton.customId) {
      const buttonInteraction = interaction as APIMessageComponentButtonInteraction;

      return await deleteMessageButton.execute({ api, env, interaction: buttonInteraction });
    }
